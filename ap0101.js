...
function init() {
  ...
  // /* 平面の設定
  const planeGeometry = new THREE.PlaneGeometry(32, 16); //平面図形を作る
  const planeMaterial = new THREE.MeshBasicMaterial({ color: 0x606060}); //素材と色の指定
  const plane = new THREE.Mesh(planeGeometry, planeMaterial);
  plane.rotation.x = -0.5 * Math.PI; //90°回す
  scene.add(plane);
  /**/
  ...
  //箱3の表示設定
  const box3Geometry = new THREE.BoxGeometry(4,8,4);
  const box3Material = new THREE.MeshLambertMaterial({color: 0xFF2000});
